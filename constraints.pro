...
gen_enforced_field(WorkspaceCwd, 'engines.node', '16.x || 18.x || 20.x').

% Required to make the package work with the GitHub Package Registry
gen_enforced_field(WorkspaceCwd, 'repository.type', 'git').
gen_enforced_field(WorkspaceCwd, 'repository.url', 'ssh://git@github.com/lukeshay/npm.git').
gen_enforced_field(WorkspaceCwd, 'repository.directory', WorkspaceCwd).

% Require scripts for all apps and packages
% gen_enforced_field(WorkspaceCwd, 'scripts.build', _).
% gen_enforced_field(WorkspaceCwd, 'scripts.test', _).
% gen_enforced_field(WorkspaceCwd, 'scripts.clean', _).

% Validate other fields
gen_enforced_field(WorkspaceCwd, 'license', 'MIT').
% gen_enforced_field(WorkspaceCwd, 'devDependencies.prettier', _).
% gen_enforced_field(WorkspaceCwd, 'devDependencies.eslint', _).
